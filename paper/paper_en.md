...
Mechanistic interpretability seeks to explain transformer behavior in terms of internal computations, but a component description is difficult to validate if it cannot be run inside the original model. We introduce **executable matrix programs**: weight-derived representations of attention and MLP components that require no retraining or learned surrogate. For RoPE attention, each head is expressed as relative-distance QK routing programs plus an affine VO payload map. The QK score can be unpacked into constant, query-affine, key-affine, and bilinear token-interaction terms. A SwiGLU MLP is expressed as a sum of gated rank-1 atoms with explicit gate, read, and write factors. These objects are derived once from the weights, reused across inputs, and reinserted into the native forward pass. On Qwen2.5-0.5B-Instruct, simultaneous replacement of all 336 attention heads and all 24 MLP sublayers preserves the top-1 prediction on 64/64 prompts with a median full-logit discrepancy of **0.258%**, validating the executable representation. The same interface supports separate interventions on QK routing, VO payload, and MLP computation. Median effects exceed the corresponding replacement discrepancy by **24.89×**, **20.62×**, and **152.05×**, respectively. Downstream profiling shows that **75.0%** of QK and **71.4%** of VO interventions peak at least four layers after the source, with median delays of **10** and **9** layers, revealing components whose main effect is preparatory rather than directly visible in final logits. The method requires an architecture-specific adapter and is evaluated on one 0.5B model and 64 short prompts. It provides an executable verification layer for claims about routing, transported payload, and MLP read/write structure.
