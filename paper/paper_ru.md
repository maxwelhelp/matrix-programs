...
Механистическая интерпретируемость стремится объяснять поведение трансформеров через их внутренние вычисления, однако описание компонента трудно проверить, если его нельзя снова выполнить внутри исходной модели. Мы представляем **исполняемые матричные программы** — полученные непосредственно из весов представления attention- и MLP-компонентов, не требующие переобучения или обучаемого суррогата. Для attention с RoPE каждая голова представляется набором QK-программ маршрутизации, зависящих от относительного расстояния, и аффинной VO-картой payload. QK-score можно раскрыть на постоянный, query-affine, key-affine и билинейный члены взаимодействия токенов. SwiGLU MLP представляется суммой gated rank-1 атомов с явными gate-, read- и write-факторами. Эти объекты один раз выводятся из весов, повторно используются на разных входах и вставляются обратно в нативный forward pass. На Qwen2.5-0.5B-Instruct одновременная замена всех 336 голов внимания и всех 24 MLP-подслоёв сохраняет top-1 предсказание на 64 из 64 промптов при медианном расхождении полного вектора логитов **0,258%**, подтверждая точность исполняемого представления. Тот же интерфейс поддерживает раздельные вмешательства в QK-маршрутизацию, VO-payload и MLP-вычисление. Медианные эффекты превышают соответствующее расхождение замены в **24,89×**, **20,62×** и **152,05×** соответственно. Downstream-профилирование показывает, что **75,0%** QK- и **71,4%** VO-вмешательств достигают максимума как минимум через четыре слоя после исходного компонента; медианные задержки составляют **10** и **9** слоёв, что выявляет компоненты, основной эффект которых является подготовительным и не виден непосредственно в финальных логитах. Метод требует адаптера для конкретной архитектуры и в этой работе проверен на одной модели размером 0,5 млрд параметров и 64 коротких промптах. Он предоставляет исполняемый слой проверки утверждений о маршрутизации, переносимом payload и read/write-структуре MLP.
